...
export const TopBarSelect = (props: TopBarSelectProps) => {
  ...
  return <>
    <Button onClick={togglePopup} isLight={isLight}>
      {props.selectedOption} <SelectChevronDown style={{ marginLeft: '11px' }} fill={isLight ? '#1AAB9B' : '#2DC1B1'} />
    </Button>
    {popup && <Popup isLight={isLight}>
      <Close
        onClick={togglePopup}
        style={{
          alignSelf: 'flex-end',
      ...
      />
      {menuItems.map((item) => <LinkWrapper isLight={isLight} key={item.title} onClick={() => setPopup(false)}><Link href={item.link}>{item.title}</Link></LinkWrapper>)}
      <Link
