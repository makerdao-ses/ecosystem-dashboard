...
const CUActivityItem2: React.FC<CUActivityItemProps> = ({ activity, isNew }) => {
...
export default CUActivityItem2;

export const ActivityItem = styled.a<{ isLight: boolean; noHover?: boolean; isGlobal: boolean }>(
  ({ isLight, isGlobal, noHover = false }) => ({
...
export const Timestamp = styled.div<{ isGlobal: boolean }>(({ isGlobal }) => ({
  display: 'flex',
...
export const Details = styled.div<{ isLight: boolean; isGlobal: boolean }>(({ isLight = true, isGlobal }) => ({
  fontWeight: 400,
  fontSize: '14px',
  lineHeight: '22px',
...
export const ButtonContainer = styled.div<{ isGlobal: boolean }>(({ isGlobal }) => ({
  textAlign: 'right',
...
export const CoreUnit = styled.div<{ isGlobal: boolean }>(({ isGlobal }) => ({
  display: isGlobal ? 'flex' : 'none',
  alignItems: 'center',
  marginTop: '18px',
...
export const FlexWrapper = styled.div<{ isGlobal: boolean }>(({ isGlobal }) => ({
  display: 'flex',
