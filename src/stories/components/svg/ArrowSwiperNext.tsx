...
interface Props {
  ...
  fillCircle?: string;
  onClick?: () => void;
  refNext?: React.LegacyRef<SVGSVGElement>;
}
  ...
  return (
    ...
    >
      <g opacity={0.6} filter="url(#filter0_d_17842_226007)">
        <circle cx={32} cy={32} r={32} transform="matrix(-1 0 0 1 70 0)" fill={fillCircle} />
      </g>
      <path
        opacity={0.8}
...
const StyledSvg = styled.svg<{ isLight: boolean }>(({ isLight }) => ({
  '&:hover circle': {
    fill: isLight ? '#E7FCFA' : '#48495F',
  },
  '&:hover path': {
    fill: isLight ? '#9FAFB9' : '#48495F',
  },
}));
